...
## Out of scope
- The release-please auth mechanism, publish-workflow tag/version consistency guard, and OIDC Trusted Publishing shape — those are separate, already-`same`-verdict or separately-owned rows; this item decides only the numeric bump-strategy semantics.
- Whether `rs-launch-blueprint` uses release-please at all as its release tool — both source repos already use it and this item assumes that choice; it decides only the pre-1.0-vs-post-1.0 bump dampening within it.
