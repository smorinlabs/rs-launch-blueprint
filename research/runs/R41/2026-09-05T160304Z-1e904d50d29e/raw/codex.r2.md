...
- The command works at the required minimum supported Rust version, abbreviated MSRV. Linux execution is an integration check still to run.
...
| Repository Bash guard | Newly authored repository code under `MIT OR Apache-2.0` | No Rust compilation; tested while invoking the actual floor toolchain | No Rust dependency tree; correctness of Git/shell handling is the relevant risk | Executed inside macOS Git commits; Linux manager integration unrun. Windows shell port is not promised | No new runtime beyond Git, Bash and the installed Cargo | Process overhead included in commit measurements |

Evidence: [main records][E], [supplemental records][O], [floor compilation output][COMPILE], [Warp workflow][WARP] and [clap workflow][CLAP], all retrieved/executed 2026-09-05. There is no blanket “all gates pass” claim. Mac and MSRV evidence are direct; Linux is an inference pending execution. Gate 2 establishes this fixture's macOS graph, not every future template dependency or every target-specific transitive crate.
