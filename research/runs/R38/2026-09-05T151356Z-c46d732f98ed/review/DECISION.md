...
re-verify: 2027-03-01, or earlier on any `committed` release beyond 1.1.11 (run the upgrade gate: re-read `checks.rs` at the new tag, then `COMMITTED_VERSION=<new> bash run-checks.sh` must exit 0), a RustSec advisory for `committed` or `git2`, R37/R42 provisioning a Node/Bun runtime for another tool, or `commitlint-rs` shipping release binaries
...
- Revision note (2026-09-05, before publication): the empirical auditor `audit-codex-2026-09-05T151356Z-c46d732f98ed` (model `gpt-5.6-terra`, family openai; `review/audit-codex.md`, verdict reject on decision sha256 `3d313fbbabef1d10ff549861d583cb25dddf82f726b58d13f47aa48eb34fe419`) reran the first fixture (exit 0, `review/evidence/audit-commit-message-linter.log`) and raised two findings, both adopted here: the whole-commit bot exemption is replaced by the width-only `committed.bot.toml` path, and the version policy is one exact pin with an executable upgrade gate (`raw/opus.md` had proposed the range `>=1.1.11, <2.0.0`).
