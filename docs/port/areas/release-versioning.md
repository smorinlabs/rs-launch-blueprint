...
| — | version consistency test suite | `tests/meta/test_version_consistency.py:56` — asserts pyproject == manifest == uv.lock == installed | `tests/version.test.ts:11` — asserts VERSION == package.json == lib re-export | same | — | py checks 4 sources incl. lockfile; ts checks 3; see F133 (same test, cited there as test_version_consistency.py:58/version.test.ts:12) |
...
| — | local packaging validation recipe | — | `Justfile:276` — `pack-check`: build, publint, attw, `npm pack --dry-run`, install smoke test | ts-only | D-012(7) | — see F221 (same `pack-check` recipe reported there against its CI-wiring cadence) |
