...
| F334 | contract fuzzing generates cases for every documented operation | `tests/web/test_contract.py:29` — `case.call_and_validate(checks=(not_a_server_error,))` | — | py-only | — | WEB-50; schemathesis, marked `slow` so the default `pytest` run skips it. |
