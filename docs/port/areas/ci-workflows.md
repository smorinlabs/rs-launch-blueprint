...
| F059 | Publish workflow triggered on a `v*` tag push | `.github/workflows/publish.yml:6` — `on:` block; `tags: ["v*"]` at :8 | `.github/workflows/publish.yml:29` — `on:` block; `- 'v*'` under `tags:` at :32 | same | D-021 | workflow trigger is CI-owned even though the publish content itself is release-versioning's |
...
| F062 | Difftree PR-comment workflow | `.github/workflows/difftree-pr-comment.yml:1` — `# Canonical consumer workflow for smorinlabs/difftree-action.` header comment | `.github/workflows/difftree-pr-comment.yml:1` — the same header comment, byte-identical | same | — | Byte-identical canonical template from smorinlabs/difftree-action in both repos |
