...
| F167 | whitespace/EOL/format-validity hygiene linter (pre-commit) | `lefthook.yml:82` — `editorconfig-checker` job runs `uv run ec -config .editorconfig-checker.json` on staged files | — | py-only | — | ts folds trailing-whitespace/EOF-newline fixing into the oxfmt formatter job instead of a separate validity linter (see lint-format.md). |
