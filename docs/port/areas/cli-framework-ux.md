...
| F275 | `--no-input` flag disables interactive prompting | `src/py_launch_blueprint/cli/options.py:116` — `"--no-input", is_flag=True` | `src/router.ts:134` — `.option('--no-input', 'never prompt; select all fetched projects')` | same | — | REUSE covers the flag's existence only; the behavioral fork under it is its own row, F360. |
| F360 | `--no-input` behavior when a prompt would have been shown | `src/py_launch_blueprint/cli/options.py:273` — `if app.no_input:` raises `ConfigError("refusing to proceed without confirmation (--no-input)…")` unless `--yes` was passed | `src/commands/projects.ts:146` — `const interactive = opts.input !== false && …`; the non-interactive path skips the prompt and selects all fetched results | different | D-016(6) | Split from F275 at the Phase 4 review: the flag is shared, the behavior under it is not. |
