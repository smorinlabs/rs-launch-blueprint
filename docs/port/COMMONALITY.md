...
| F212 | build backend/bundler tool | packaging-distribution | same | COMMON → SUBSTITUTE | R49 | parent: F211; uv_build (PEP 517 backend) and tsdown (rolldown bundler) are both language-bound third-party tools; Cargo is Rust's native build system with no separate backend/bundler crate needed — decides F213, F216, F218, F219, F220 |
...
| F214 | distributable artifact types | packaging-distribution | different | DIVERGENT | R49 | bundle; py ships sdist+wheel, ts ships one ESM npm tarball — `cargo publish` produces one `.crate` file; whether rs-launch-blueprint also ships cross-compiled binary release artifacts (a pattern neither source repo has) is the open question |
...
| F220 | documented end-user install method | packaging-distribution | py-only | DIVERGENT | R49 | bundle; which install command(s) the README documents (`cargo install`, `cargo binstall`, or a downloaded release binary) follows directly from F214's artifact-shape decision |
...
| F223 | container image as a distribution artifact | packaging-distribution | py-only | DIVERGENT | R51 | standalone; contingent on F017 (workspace-architecture, already ADOPTed: web service as an optional Cargo feature) — whether the optional web-service build also gets a Dockerfile/multi-stage image |
