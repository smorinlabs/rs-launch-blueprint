...
| F183 | per-tool binary installer recipes for tools outside the primary package manager | dev-experience-repo-hygiene | py-only | DIVERGENT | R42 | bundle; whether Rust needs Justfile installer recipes for non-cargo dev tools, or a declarative provisioner instead — decides F186-F189 |
...
| F186 | hook-toolchain readiness check (doctor for git-hook tools) | dev-experience-repo-hygiene | py-only | DIVERGENT | R42 | bundle |
...
| F195 | VS Code debugger tool/extension for the CLI | dev-experience-repo-hygiene | same | COMMON → SUBSTITUTE | R30 | parent: F194; bundle, decided together with F105-F113 (static-analysis's type-checker/editor-tooling bundle) — Rust's debugger extension (e.g. CodeLLDB) is commonly set up alongside rust-analyzer |
| F196 | committed `.vscode/settings.json` pinning the editor's language-service version | dev-experience-repo-hygiene | ts-only | DIVERGENT | R30 | bundle |
