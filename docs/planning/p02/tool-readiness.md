...
| Create-retry isolation | Unpatched: Doxa wraps every provider submit in three tenacity attempts, the OpenAI SDK adds `DEFAULT_MAX_RETRIES = 2` and its `_idempotency_header` is `None` (keys are generated but never sent), google-genai defaults to one attempt, and Perplexity's stable idempotency key has documented purpose but undocumented deduplication semantics. `scripts/doxa_no_retry.py` rebinds all five submit wrappers to one attempt and forces `max_retries = 0`; `--verify` prints the effective values and `scripts/test-doxa-no-retry.py` (4 tests) proves the patched and unpatched states differ | `probes/doxa-no-retry-verify.json` |
